        ...
        if 'results' in data:
            for track in data['results']:
                if artist.lower() in track.get('artistName', '').lower():
                    ...
                    # Apple Music meletakkan maklumat Composition & Lyrics di bawah 'composerName'
                    composition_lyrics = track.get('composerName', 'N/A')
                    ...
                    data_list.append({
                        ...
                        'Cover': cover_url,
                        'Composition & Lyrics': composition_lyrics
                    })
    except Exception as e:
        print(f"Ralat pada artis {artist}: {e}")

df = pd.DataFrame(data_list).drop_duplicates(subset=['ID Lagu'])
df.to_csv('munsyid_data.csv', index=False)
print(f"Berjaya! {len(df)} lagu disimpan.")
